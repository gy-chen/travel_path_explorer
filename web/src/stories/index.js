...
import {
  ...
  connectAppLocalizationProvider
} from "../i18n";

import {
  explore as exploreApi,
  geolocation as geolocationApi
} from "../service";

import { withRouteData } from "./moc/withRouteData";

storiesOf("App", module).add("Basic", () => {
  const store = configureStore();

  return (
    <Provider store={store}>
      <GoogleStaticMapKeyContext.Provider
        value={process.env.STORYBOOK_GMAPS_API_KEY}
      >
...
storiesOf("Intro", module).add("Basic", () => (
  <Router>
    <Intro />
  </Router>
));

storiesOf("Report", module).add("Basic", () => {
...
storiesOf("Explore", module)
  ...
  .add("Error: Service Unavailable", () => (
    <Explore currentError={exploreApi.STATUS_CODE.UNAVAILABLE} />
  ))
  .add("Error: Not Found", () => (
    <Explore currentError={exploreApi.STATUS_CODE.NOT_FOUND} />
  ))
  .add("Fetched", () => {
...
storiesOf("Error", module)
  .add("Unavailable", () => (
    <Error errorCode={exploreApi.STATUS_CODE.UNAVAILABLE} />
  ))
  .add("Not found", () => (
    <Error errorCode={exploreApi.STATUS_CODE.NOT_FOUND} />
  ));

storiesOf("GoogleMap", module)
  .add("Basic", () => (
    <GoogleMap center={{ lat: -34.397, lng: 150.644 }} zoom={8} />
  ))
  .add("SearchBox", () => (
    <GoogleMap
      enableSearchBox
...
storiesOf("GoogleMapPlacePicker", module).add("Basic", () => (
...
storiesOf("PlacePickerCard", module).add("Basic", () => (
  <PlacePickerCard
...
storiesOf("Step", module).add("Basic", () => {
...
storiesOf("Parking", module).add("Basic", () => {
...
storiesOf("GoogleStaticMap", module)
  .add("Path", () => (
    <GoogleStaticMapKeyContext.Provider
      value={process.env.STORYBOOK_GMAPS_API_KEY}
    >
      <GoogleStaticMap path="knirCqpr_V?uA@a@BaAEcBiAoDSo@a@oAIQEQI_@o@RwAV}AV_@Jg@@eEGeDEeCCAT{AE}@CeCKU`IUxGCf@`@H" />
    </GoogleStaticMapKeyContext.Provider>
  ))
  .add("Center", () => (
    <GoogleStaticMapKeyContext.Provider
      value={process.env.STORYBOOK_GMAPS_API_KEY}
    >
      <GoogleStaticMap center={{ lat: 24.138877, lng: 120.6895124 }} />
    </GoogleStaticMapKeyContext.Provider>
  ));

storiesOf("Redux State", module).add("Basic", () => {
  const DevTools = createDevTools(<LogMonitor />);
        ...
        <Wrapper>
          <ButtonWrapper>
            <button
              onClick={() =>
                store.dispatch(
                  route.fetchRoute("Taichung Train Station", "Taichung Park")
                )
              }
            >
              Fetch Route Action
            </button>
            <button onClick={() => store.dispatch(route.requestRoute())}>
            ...
            <button
              onClick={() => store.dispatch(route.receiveError("NOT_FOUND"))}
            >
...
storiesOf("Service", module)
  .add("Explore", () => {
    ...
    class ExploreStory extends React.Component {
      ...
      onExplore() {
        exploreApi
          .findRoute(this.state.origin, this.state.destination)
          .then(response => {
            this.setState({
      ...
      render() {
        return (
          <Wrapper>
            ...
            <ApiResult>{this.state.apiResult}</ApiResult>
          </Wrapper>
        );
      }
    }
  ...
  .add("Geolocation", () => {
    class GeolocationStory extends React.Component {
      constructor(props) {
        ...
        this._onGetCurrentGeolocationClick = this._onGetCurrentGeolocationClick.bind(
          this
        );
      }
      ...
      render() {
        return (
          <div>
            <button onClick={this._onGetCurrentGeolocationClick}>
              Get Current Geolocation
            </button>
            <pre>{this.state.apiResult}</pre>
          </div>
        );
      }
    }
